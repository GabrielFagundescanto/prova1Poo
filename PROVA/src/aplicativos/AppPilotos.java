...
public class AppPilotos {
    public static void main(String[] args) throws InterruptedException, IOException {
        ...
        Pessoa [] VetorX = new Pessoa[MAX_ELEMENTOS];
        int NOVO_MAX_ELEMENTOS = MAX_ELEMENTOS;
    

        do {

            System.out.println("O valor do armazenamento do piloto é: " + MAX_ELEMENTOS);
            System.out.println("O novo valor do armazenamento é: " + NOVO_MAX_ELEMENTOS);
            System.out.println("\n**\nMENU\n**\n");
            ...
            if (opcao == 1) {
                ...
                if (qtdCadastrados == NOVO_MAX_ELEMENTOS) {
                ...
                piloto.setNome(nome); 
                piloto.setCpf(cpf);
                piloto.getPilotoA().setMatricula(matricula);
                piloto.getPilotoA().setBreve(breve); 
                ...
                System.out.println("Qual o tamanho do novo armazenamento: ");
                int valorDigitado = in.nextInt();

                NOVO_MAX_ELEMENTOS = valorDigitado;

                VetorX = new Pessoa[NOVO_MAX_ELEMENTOS];
                for (int i = 0; i < pilotos.length; i++) {    // Armazena a quantidade de armazenamento em uma variavel temporaria
                    VetorX[i] = pilotos[i];                         
                }

                pilotos = new Pessoa[NOVO_MAX_ELEMENTOS];
                for (int i = 0; i < VetorX.length; i++) {     // Utiliza a quantidade armazenada pela variavel anterior
                    pilotos[i] = VetorX[i];
